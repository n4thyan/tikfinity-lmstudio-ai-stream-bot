...
A working local AI livestream bot that connects live chat to a small LM Studio model, displays replies in OBS, speaks them with browser TTS, and logs activity to Discord.

## Project type

AI-assisted livestream automation project.

## Problem

I wanted to build a real stream tool where viewers could interact with a local AI character instead of relying on a hosted chatbot API. The goal was to make something practical, funny, and technically useful: a small AI character that could run locally, appear on stream, react to commands, and be controlled safely enough for a public livestream.
...
Portfolio v1 is complete and released as a working local project. Further improvements could include a stronger hosting setup, more stream events, better model tuning, more advanced moderation rules, and a polished public demo video.
